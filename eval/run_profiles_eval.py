...
def fetch_clean(**overrides):
    ...
    data = {
        ...
        "санкции": {"статус": "проверено", "совпадений": 0, "совпадения": [],
                    "дата_проверки": ДАТА},
        # браузерные блоки — по контракту ручного блока (references/brauzer.md)
        "фссп": {"статус": "проверено", "производств": 0, "сумма": 0, "крупные": False,
                 "ввод": "браузер", "дата_проверки": "2026-09-19",
                 "url": "https://fssp.gov.ru/iss/ip"},
        "суды": {"статус": "проверено", "ответчик_дел": 0, "ответчик_сумма": 0,
                 "ответчик_крупные": False, "ввод": "браузер",
                 "дата_проверки": "2026-09-19", "url": "https://kad.arbitr.ru/"},
...
def case_o_fedresurs(pf):
    """Федресурс из живых фикстур: намерение кредитора — 🔴 для отсрочки, 🟡 для
    предоплаты; ликвидация — стоп; компания-кредитор — без сигнала."""
    errors = []
    net = ROOT / "eval" / "fixtures" / "net"

    def с_блоком(inn):
        exp = json.loads((net / ("федресурс_%s.expected.json" % inn)).read_text(encoding="utf-8"))
        fetch = fetch_clean(федресурс=exp["данные"])
        fetch["_доступность"]["федресурс"] = _av("ok", tier="🟡")
        return fetch
    for inn, профиль, цвет, сигнал in (
            ("5029169023", "отсрочка", "🔴", "намерение_кредитора_банкротство"),
            ("5029169023", "предоплата", "🟡", "намерение_кредитора_банкротство"),
            ("9500007240", "отсрочка", "🔴", "ликвидация"),
            ("7713392265", "отсрочка", "🔴", "недостоверность_сведений"),
            ("7707083893", "отсрочка", "🟢", None)):
        res = pf.resolve(с_блоком(inn), fin_ok(), профиль)
        поднят = [x.get("сигнал") for x in res.get("поднят_сигналами", [])]
        check(errors, res.get("светофор") == цвет and (сигнал is None or сигнал in поднят),
              "Федресурс %s/%s: %r %r, ждали %s %s" % (inn, профиль, res.get("светофор"),
                                                      поднят, цвет, сигнал))
    # собственное намерение должника — 🔴 даже для предоплаты
    fetch = с_блоком("9500007240")
    fetch["федресурс"] = dict(fetch["федресурс"], решение_о_ликвидации=False,
                              намерение_должника="2026-09-01: намерение должника (сообщение 1)")
    res = pf.resolve(fetch, fin_ok(), "предоплата")
    check(errors, res.get("светофор") == "🔴" and "намерение_должника_банкротство" in
          [x.get("сигнал") for x in res.get("поднят_сигналами", [])],
          "намерение должника: %r %r" % (res.get("светофор"), res.get("поднят_сигналами")))
    # усечённый Федресурс при чистом реестре: ликвидация «отсутствует» по реестру
    # (первичный источник), но факт помечен неполным — не «доказанное отсутствие»
    fetch = с_блоком("7707083893")
    факт = [x for x in pf.resolve(fetch, fin_ok(), "отсрочка").get("факты", [])
            if x.get("сигнал") == "ликвидация"]
    check(errors, факт and факт[0].get("статус") == "отсутствует"
          and "федресурс" in str(факт[0].get("неполно")),
          "ликвидация при усечённом Федресурсе: пометка «частично»: %r" % факт)
    return errors


def case_p_manual_contract(pf):
    """Ручной блок (браузер) засчитывается, только если заявлен проверенным и у
    него есть ISO-дата и http(s)-URL; своё «не проверено» побеждает всегда."""
    errors = []
    полный = {"ввод": "браузер", "дата_проверки": "2026-09-26", "url": "https://kad.arbitr.ru/"}
    база = {"статус": "проверено", "ответчик_дел": 0, "ответчик_сумма": 0,
            "ответчик_крупные": False}
    не_пр = _av("не проверено", "не покрыто: kad.arbitr — браузер", tier="🔴", требует="браузер")
    ок = _av("ok", tier="🔴", требует="браузер")
    for имя, блок, av, ждём_ok in (
            ("без даты и url", dict(база), не_пр, False),
            ("только дата", dict(база, дата_проверки="2026-09-26"), не_пр, False),
            ("только url", dict(база, url="https://kad.arbitr.ru/"), не_пр, False),
            ("_доступность ok, но без даты и url", dict(база), ок, False),
            ("честное «не проверено» с датой и url",
             dict(база, **полный, статус="не проверено: капча не решена"), не_пр, False),
            ("без статуса, с датой и url",
             {k: v for k, v in dict(база, **полный).items() if k != "статус"}, не_пр, False),
            ("дата не ISO", dict(база, **dict(полный, дата_проверки="вчера")), не_пр, False),
            ("дата ДД.ММ.ГГГГ", dict(база, **dict(полный, дата_проверки="26.09.2026")), не_пр, False),
            ("дата в будущем", dict(база, **dict(полный, дата_проверки="2099-01-01")), не_пр, False),
            ("дата с хвостом", dict(база, **dict(полный, дата_проверки="2026-09-26 вроде")), не_пр, False),
            ("«проверено частично» при ok скрипта",
             dict(база, **полный, статус="проверено частично"), ок, False),
            ("без статуса, ok скрипта, по контракту",
             {k: v for k, v in dict(база, **полный).items() if k != "статус"}, ок, True),
            ("дата несуществующая", dict(база, **dict(полный, дата_проверки="2026-02-30")), не_пр, False),
            ("url не http", dict(база, **dict(полный, url="ftp://x")), не_пр, False),
            ("полный контракт", dict(база, **полный, параметры_поиска="ИНН 7707083893",
                                     просмотрено="1 страница", проверил="пользователь"), не_пр, True),
            ("полный контракт, _доступность ok", dict(база, **полный), ок, True),
            ("поля контракта в _доступность", dict(база),
             dict(не_пр, ввод="браузер", дата_проверки="2026-09-26",
                  url="https://kad.arbitr.ru/"), True)):
        fetch = fetch_clean(суды=блок)
        fetch["_доступность"]["суды"] = av
        состояние, причина, _ = pf._состояние_блока(fetch, "суды")
        check(errors, (состояние == "ok") == ждём_ok,
              "%s: %s (%s)" % (имя, состояние, причина))
        if not ждём_ok and "статус" in блок and not блок["статус"].startswith("не проверено"):
            check(errors, "references/brauzer.md" in str(причина),
                  "%s: отказ без ссылки на контракт: %r" % (имя, причина))
    # небраузерный источник: своё «не проверено» бьёт ok скрипта, а заявка
    # «проверено» поверх молчавшего скрипта — только по тому же контракту
    санкции = {"статус": "проверено", "совпадений": 0, "совпадения": []}
    for имя, блок, av, ждём_ok in (
            ("санкции: своё «не проверено» при ok скрипта",
             dict(санкции, статус="не проверено: дамп устарел"), _av("ok"), False),
            ("санкции: «проверено частично» при ok скрипта",
             dict(санкции, статус="проверено частично"), _av("ok"), False),
            ("санкции: «проверено» вручную без даты и url",
             dict(санкции), _av("не проверено", "гео: fedsfm"), False),
            ("санкции: «проверено» вручную по контракту",
             dict(санкции, дата_проверки="2026-09-26", url="https://fedsfm.ru/"),
             _av("не проверено", "гео: fedsfm"), True)):
        fetch = fetch_clean(санкции=блок)
        fetch["_доступность"]["санкции"] = av
        состояние, причина, _ = pf._состояние_блока(fetch, "санкции")
        check(errors, (состояние == "ok") == ждём_ok, "%s: %s (%s)" % (имя, состояние, причина))
    # и светофор: без даты/URL суды не проверены — 🟢 не выдаётся
    fetch = fetch_clean(суды=dict(база))
    res = pf.resolve(fetch, fin_ok(), "отсрочка")
    check(errors, res.get("светофор") != "🟢", "ручной блок без даты дал 🟢: %r" % res.get("светофор"))
    check(errors, "суды" in str(res.get("рекомендация")),
          "рекомендация не называет непроверенные суды: %r" % res.get("рекомендация"))
    return errors
...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        cases = {
            ...
            "о-федресурс-из-фикстур": case_o_fedresurs(pf),
            "п-ручной-блок-контракт": case_p_manual_contract(pf),
        }
    failed = 0
