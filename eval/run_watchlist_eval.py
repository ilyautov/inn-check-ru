...
директории (INN_CHECK_WATCHLIST / INN_CHECK_CACHE). Проверяется:
  ...
  2. --прогон делает отпечаток (snapshot.ВЕРСИЯ_ФОРМАТА), первый прогон — «первый снимок»;
...
def _модуль_снимков():
    """scripts/snapshot.py как модуль — чтобы версия формата не дублировалась числом."""
    spec = importlib.util.spec_from_file_location(
        "snapshot_for_eval", str(ROOT / "scripts" / "snapshot.py"))
...
def кейс_прогон(окружение):
    ...
    if отчёт:
        ...
        if not снимок or not os.path.exists(снимок):
            ...
            актуальная = _модуль_снимков().ВЕРСИЯ_ФОРМАТА
            if отпечаток.get("версия_формата") != актуальная:
                errors.append("снимок не отпечаток: версия_формата %r"
                              % отпечаток.get("версия_формата"))
            for ключ in ("поля", "вердикт", "состояния", "сбор",
                         "хеш_остального", "_итог"):
