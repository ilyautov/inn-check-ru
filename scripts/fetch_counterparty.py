...
def parse_egrul(raw, inn):
    ...
    card = {
        ...
        "адрес": None,
        "адрес_источник": "не отдаётся egrul search-result; " + (
            "регион: %s" % region if region else "регион в строке поиска не указан"),
        "статус": None,
...
def fetch_opendata_dumps(opener, inn):
    ...
    долг = по_датасетам.get("debtam") or {}
    штр = по_датасетам.get("taxoffence") or {}
    ...
    # Недоимка: запись есть и сумма > 0 — сигнал «налоговая_задолженность» с
    # датой выгрузки. Отсутствие записи НЕ ставит False: в дампы попадают не все,
    # и «нет записи» — факт о выгрузке, а не о компании.
    if долг.get("состояние") == "ок":
        данные["недоимка_руб"] = {
            "всего": долг.get("недоимка_всего_руб"),
            "налог": долг.get("недоимка_налог_руб"),
            "пени": долг.get("пени_руб"),
            "штрафы": долг.get("штрафы_руб"),
        }
        данные["недоимка_на_дату"] = долг.get("на_дату")
        данные["недоимка_оговорка"] = ("на дату состояния, без учёта погашения "
                                       "после неё (паспорт debtam)")
        if (долг.get("недоимка_всего_руб") or 0) > 0:
            # Строкой, а не True: в карточке сигнала видны сумма и дата выгрузки.
            данные["налоговая_задолженность"] = (
                "недоимка %.2f руб. на %s (выгрузка ФНС debtam от %s)"
                % (долг["недоимка_всего_руб"], долг.get("на_дату") or "дату не указана",
                   долг.get("дата_выгрузки") or "?"))
    if штр.get("состояние") == "ок":
        # Не «все штрафы за год»: решения, вступившие в силу в году `год`, со
        # штрафом, не уплаченным к 1 октября следующего (паспорт taxoffence).
        # С недоимкой из debtam не складывается — наборы пересекаются.
        данные["неуплаченные_налоговые_штрафы_руб"] = штр.get("штрафы_руб")
        данные["налоговые_штрафы_решения_вступили_год"] = штр.get("год")
