...
SOURCES = {
    ...
    "росстат": {
        "название": "Статрегистр Росстата (websbor): ОКПО, ОКФС, ОКОПФ, подразделения",
        "tier": "🟢",
        "требует": "сеть",
        "deal_killer": False,
        "фаза": "досье",
        "профили": "*",
        "probe": {"url": "https://websbor.rosstat.gov.ru/online/info", "method": "GET",
                  "ok_http": [200], "ожидаем": "html"},
        "контракт": ["inn", "okpo", "type", "okfs"],
        "канарейка": {"инн": "7707083893", "ожидаем_непустые": ["окпо"]},
        "при_отказе": "не проверено",
        "документация": "https://websbor.rosstat.gov.ru/online/info",
    },
    "список_цб": {
        "название": "Список ЦБ: признаки нелегальной деятельности на финрынке (cbr_warning.py, кэш)",
        "tier": "🟡",
        "требует": "кэш",
        "deal_killer": False,
        "фаза": "quick",
        "профили": "*",
        "probe": {"url": "https://www.cbr.ru/inside/warning-list/", "method": "GET",
                  "ok_http": [200], "ожидаем": "html"},
        "контракт": [],
        "канарейка": None,
        "при_отказе": "не проверено",
        "документация": "https://www.cbr.ru/inside/warning-list/",
        # «нет совпадения» — не «нет в списке»: ИНН есть лишь у части записей.
        # Текст постоянный (без дат и счётчиков выгрузки): профиль берёт его
        # отсюда, поэтому ретро по снимку печатает ту же оговорку, что и живой
        # прогон, а обновление кэша не даёт диффу ложного «что-то изменилось».
        "пусто_с_оговоркой": ("ИНН указан лишь у части записей списка ЦБ, сверка — "
                              "по точному ИНН, поэтому отсутствие совпадения не "
                              "значит, что компании в списке нет; выгрузка не "
                              "старше 14 дней"),
    },
    "санкции": {
    ...
    "федресурс": {
        "название": "Федресурс — сообщения о фактах деятельности (fedresurs.ru)",
        "tier": "🟡",
        # Та же политика JSON сайтов, что у ЕФРСБ: два запроса (поиск по ИНН и первая
        # страница публикаций), Referer страницы сайта, UA проекта, без повторов.
        # Сигналы — только по типам, снятым с живых ответов 26.09.2026.
        "требует": "сеть",
        "deal_killer": False,
        "фаза": "досье",
        "профили": "*",
        "probe": {"url": "https://fedresurs.ru/backend/companies"
                         "?searchString=7707083893&limit=15&offset=0",
                  "method": "GET", "ok_http": [200], "ожидаем": "json",
                  "referer": "https://fedresurs.ru/"},
        "контракт": ["inn", "guid"],
        # Сбербанк публикует намерения кредитора о чужом банкротстве (роль —
        # публикатор): список сообщений непустой и без сигнала.
        "канарейка": {"инн": "7707083893", "ожидаем_непустые": ["сообщения"]},
        "при_отказе": "не проверено",
        "документация": "https://fedresurs.ru/",
    },
}
