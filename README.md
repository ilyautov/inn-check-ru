...
**Санкционный контур — офлайн.** `scripts/sanctions_check.py` сверяет директора и компанию с перечнем Росфинмониторинга локально за секунды (перечень скачивается один раз в кэш, сверка по ИНН точная, по ФИО — с пометкой «проверьте дату рождения»). Плюс чеклист однодневки по методике ФНС из уже собранных сигналов (3+ признака → 🟡, 5+ → 🔴) и ранние сигналы ликвидации из Вестника госрегистрации и Федресурса — они попадают в ЕГРЮЛ с опозданием в недели.
...
Оценка риска по открытым данным, а не юридическая или кредитная гарантия. Для крупной или необратимой сделки светофор — рабочий лист для юриста и финансового аналитика, не последнее слово.
