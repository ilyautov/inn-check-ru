...
**Цель проверки задаёт вес фактов.** Восемь профилей в `data/profiles_ru.json` (`нейтрально` по умолчанию, `отсрочка`, `предоплата`, `подрядчик`, `доля`, `клиент_115фз`, `самопроверка`, `ндс_вычет`): отрицательные чистые активы — стоп для отгрузки в долг и аргумент в торге при покупке доли. Светофор и рекомендацию считает `scripts/profiles.py` из собранного JSON, только вверх по тревожности; без названной цели — карточка фактов без вердикта. `fetch_counterparty.py <ИНН> --профиль отсрочка` собирает только то, что влияет на вердикт в этом профиле.
...
- **MCP-сервер (`mcp/`)** — те же скрипты как 13 read-only инструментов MCP-сессии. Когда проверка — шаг в цепочке инструментов: счёт из diadoc-mcp-ru → `counterparty_fetch` до подписи. Tier-маркеры и «не проверено» проезжают в ответах как есть. Установка и конфиги: [mcp/README.md](mcp/README.md).
