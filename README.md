...
[![Версия](https://img.shields.io/badge/версия-1.8.0-blueviolet)](CHANGELOG.md)
...
**Цель проверки задаёт вес фактов.** Семь профилей в `data/profiles_ru.json` (`нейтрально` по умолчанию, `отсрочка`, `предоплата`, `подрядчик`, `доля`, `клиент_115фз`, `самопроверка`): отрицательные чистые активы — стоп для отгрузки в долг и аргумент в торге при покупке доли. Светофор и рекомендацию считает `scripts/profiles.py` из собранного JSON, только вверх по тревожности; без названной цели — карточка фактов без вердикта. `fetch_counterparty.py <ИНН> --профиль отсрочка` собирает только то, что влияет на вердикт в этом профиле.
