...
- **Скилл (`SKILL.md`)** — методология для агента: двухскоростной флоу, каскад источников, светофор, мониторинг. Когда проверка — часть живого разговора («проверь этого поставщика перед предоплатой»).
- **MCP-сервер (`mcp/`)** — те же скрипты как 6 read-only инструментов MCP-сессии. Когда проверка — шаг в цепочке инструментов: счёт из diadoc-mcp-ru → `counterparty_fetch` до подписи. Tier-маркеры и «не проверено» проезжают в ответах как есть. Установка и конфиги: [mcp/README.md](mcp/README.md).
