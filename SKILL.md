...
| 🟢 Спецреестры ФНС | дисквалификация руководителя, налоговая задолженность >1000 ₽, недостоверность сведений | скрипт (service.nalog.ru; эндпоинты не верифицированы с не-РФ IP) | без ключа |
| 🟢 Санкции | перечень РФМ (террористы/экстремисты) + OFAC SDN (США) + EU Consolidated (ЕС) — директор и компания | офлайн-сверка кэшем (`scripts/sanctions_check.py`), секунды | без ключа |
...
1. **Снимок перед каждой крупной отгрузкой:** `python3 scripts/fetch_counterparty.py <ИНН> --save` — сохраняет JSON проверки в `~/.cache/inn-check-ru/snapshots/<ИНН>/<дата>_<время>.json`.
