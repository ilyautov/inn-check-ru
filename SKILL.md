...
| 🟢 Санкции РФМ | перечень террористов/экстремистов — директор и компания | офлайн-сверка кэшем (`scripts/sanctions_check.py`), секунды | без ключа |
...
- **санкционная сверка** директора и компании по перечню Росфинмониторинга — офлайн, секунды: `python3 scripts/sanctions_check.py --inn <ИНН> --name "<ФИО директора>"` (кэш перечня обновляется `--refresh`);
...
## Связки
- `scripts/fetch_counterparty.py` — сбор зелёной зоны ФНС (ЕГРЮЛ / Прозрачный бизнес / ГИР БО + Реестр МСП и спецреестры service.nalog.ru, последние — не верифицированы с не-РФ IP).
- `scripts/fin_scoring.py` — финансовый профиль кодом из строк ГИР БО (пороги в разделе «Финансовый профиль кодом»).
- `scripts/sanctions_check.py` — офлайн-сверка с перечнем Росфинмониторинга (кэш в `~/.cache/inn-check-ru/`).
