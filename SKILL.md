...
**Пороги — из канона, не из кода:** `data/canon_ru.json` — каждое число с источником и датой проверки (освобождение от НДС на УСН 20 млн ₽ в 2026, 15 млн в 2027, 10 млн в 2028; лимит УСН 490,5 млн; переходный 337,5 млн; проверено 19.09.2026). Канон отсутствует/битый → блок выручки честно «не проверено».
...
- `scripts/profiles.py` + `data/profiles_ru.json` — профили цели (нейтрально / отсрочка / предоплата / подрядчик / доля / клиент_115фз / самопроверка): единый каталог сигналов, deal-killers и рекомендации по профилю, светофор кодом (шаги 1 и 5); `--list` — таблица профилей.
