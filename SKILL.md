...
| 🟢 Реестр МСП | статус МСП, категория (микро/малое/среднее), численность | скрипт (rmsp.nalog.ru; эндпоинт не верифицирован с не-РФ IP) | без ключа |
...
Скрипт зелёной зоны: `scripts/fetch_counterparty.py <ИНН>` → JSON по трём источникам ФНС. Транспортные грабли: kad.arbitr за DDoS-Guard (голый скрипт = 451, нужен браузер); ФССП агрессивно лимитит (вежливые задержки, токен); эндпоинты ФНС недокументированы (могут смениться).
...
## Связки
- `scripts/fetch_counterparty.py` — сбор зелёной зоны ФНС (ЕГРЮЛ / Прозрачный бизнес / ГИР БО + Реестр МСП и спецреестры service.nalog.ru, последние — не верифицированы с не-РФ IP); `--save` — снимок для мониторинга.
- `scripts/fin_scoring.py` — финансовый профиль кодом из строк ГИР БО (пороги в разделе «Финансовый профиль кодом»).
